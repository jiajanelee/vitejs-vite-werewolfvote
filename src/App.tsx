...
const FIREBASE_URL =
  'https://werewolf-voting-default-rtdb.asia-southeast1.firebasedatabase.app';
const FIREBASE_CONFIGURED =
  FIREBASE_URL !== 'YOUR_DATABASE_URL' && FIREBASE_URL.startsWith('https://');
...
async function saveRoom(room) {
  ...
  if (!res.ok)
    throw new Error(`Firebase 儲存失敗：${res.status} ${res.statusText}`);
}
...
function computeNightDeaths(night, lastDreamTarget) {
  ...
  if (dreamerNum) {
    ...
    if (dreamerWolfKilled) {
      const singleGuard =
        guardTarget === dreamerNum && witchSave !== dreamerNum;
      const singleWitch =
        witchSave === dreamerNum && guardTarget !== dreamerNum;
  ...
  if (
    witchPoison &&
    witchPoison !== dreamTarget &&
    witchPoison !== dreamerNum
  ) {
...
function GodNightPanel({ room, godAction, loading }) {
  ...
  const buildOrder = () => {
    ...
    if (!isFirstNight && room.lonegirlTransformed && room.lonegirlNewRole) {
      ...
      const stepMap = {
        seer: {
          roleId: 'seer',
          label: '覺醒孤獨少女（繼承預言家）睜眼',
          identifyPlayers: false,
          action: { key: 'check', label: '請選擇查驗目標', multi: false },
        },
        witch: {
          roleId: 'witch',
          label: '覺醒孤獨少女（繼承女巫）睜眼',
          identifyPlayers: false,
          action: {
            key: 'witch',
            label: '女巫行動',
      ...
      if (injected) {
        ...
        base = base.map((s, i) =>
          i === existIdx
            ? { ...s, label: injected.label, note: injected.note }
            : s
        );
  ...
  const advanceStep = async () => {
    ...
    if (currentStep?.roleId === 'lucky') {
      ...
      stepData.luckyIsWolf = luckyRole
        ? ROLE_MAP[luckyRole]?.camp === 'wolf'
        : false;
      ...
      {currentStep ? (
        (() => {
          try {
            ...
            const curPreset =
              PRESETS.find((p) => p.id === room.preset) || PRESETS[0];
            const hasWolf2 = curPreset.roles.includes('wolf2');
            const wolfCount = curPreset.roles.filter(
              (rid) => rid === 'wolf'
            ).length;
            ...
            return (
              ...
              >
                ...
                {currentStep.firstNightOnly && (
                  <div
                    style={{ fontSize: 11, color: clr.warn, marginBottom: 6 }}
                  >
                    ★ 僅首夜行動
                  </div>
                )}
                        ...
                        {hasWolf2 && (
                          <div>
                            ...
                            >
                              ...
                              {wolf2Sel.length > 1 && (
                                <span
                                  style={{ color: clr.danger, marginLeft: 6 }}
                                >
                                  ⚠ 只能選 1 人
                                </span>
                              )}
                            ...
                            <NightPickBtn
                              ...
                              multi={true}
                              label=""
                              opts={aliveNums.filter(
                                (n) => !wolfSel.includes(n)
                              )}
                              na={na}
                  ...
                  >
                    ...
                    {act.isWitch &&
                      (() => {
                        ...
                        const witchKilled =
                          wolfKill &&
                          witchNum &&
                          Number(wolfKill) === Number(witchNum);
                        const canSave =
                          wolfKill && !witchKilled && !room.witchSaveUsed;
                          ...
                          <div>
                            ...
                            >
                              <span
                                style={{
                                  fontSize: 12,
                                  color: room.witchSaveUsed
                                    ? clr.danger
                                    : clr.success,
                                }}
                              >
                                💊 解藥：
                                {room.witchSaveUsed ? '已用完' : '剩餘 1 次'}
                              </span>
                              <span
                                style={{
                                  fontSize: 12,
                                  color: room.witchPoisonUsed
                                    ? clr.danger
                                    : clr.success,
                                }}
                              >
                                ☠ 毒藥：
                                {room.witchPoisonUsed ? '已用完' : '剩餘 1 次'}
                              </span>
                            </div>
                            <div
                              style={{
                    ...
                    {act.isSwap &&
                      (() => {
                        ...
                        const swapOpts = aliveNums.filter(
                          (n) => !usedNums.includes(n)
                        );
                        return (
                          <div>
                            {usedNums.length > 0 && (
                    ...
                    {currentStep.roleId === 'lucky' &&
                      ...
                      (() => {
                        const luckyNum = room.night?.grant?.[0] || null;
                        const luckyRole = luckyNum
                          ? room.roles?.[`p${luckyNum}`]
                          : null;
                        const luckyIsWolf = luckyRole
                          ? ROLE_MAP[luckyRole]?.camp === 'wolf'
                          : false;
                          ...
                          <div>
                            ...
                            {luckyNum && !luckyIsWolf && (
                              <NightPickBtn
                                ...
                                label={`幸運兒使用技能目標（${
                                  GRANT_SKILLS.find(
                                    (g) => g.key === room.night?.grantSkill?.[0]
                                  )?.label || '技能'
                                }）`}
                                opts={aliveNums}
                                na={na}
                    ...
                    {act.isGuard && (
                      <div>
                        ...
                        <button
                          onClick={() =>
                            setNightActions((prev) => ({ ...prev, guard: [] }))
                          }
                          style={{
                            marginTop: 8,
                            ...btn(
                              !na.guard || na.guard.length === 0
                                ? 'warn'
                                : 'default'
                            ),
                            fontSize: 12,
                          ...
                          {currentStep.roleId === 'seer' &&
                            ...
                            (() => {
                              ...
                              if (!roleId && isFirstNight) {
                                ...
                                // 其他角色的 id_* key
                                const allRoleIds = Object.keys(ROLE_MAP).filter(
                                  (r) => r !== 'wolf' && r !== 'wolf2'
                                );
                                if (wolf2List.includes(realTarget))
                                  roleId = 'wolf2';
                                else if (wolfList.includes(realTarget))
                                  roleId = 'wolf';
                                else {
                ...
                {!act &&
                  ...
                  (() => {
                    ...
                    const swap =
                      room.night?.swap?.length === 2 ? room.night.swap : null;
                    // 女巫毒藥目標（原始，未 swap）
                    ...
                    const hunterNum = isFirstNight
                      ...
                      : roleOwners('hunter')[0];
                    // 獵人被毒殺：毒藥實際打到獵人
                    const hunterPoisoned =
                      hunterNum && poisonReal === hunterNum;
                    ...
                    return (
                      <div
                        style={{
                          ...
                          border: `0.5px solid ${
                            canShoot ? clr.success : clr.danger
                          }`,
                        }}
                      >
                        <div
                        ...
                        {hunterPoisoned && (
                          ...
                          >
                            女巫毒藥命中獵人（{hunterNum} 號）
                            {swap && poisonRaw !== poisonReal
                              ? `，經魔術師互換（原目標 ${poisonRaw} 號）`
                              : ''}
                          </div>
                        )}
                        {!hunterNum && (
                ...
                >
                  ...
                  {stepIdx > 0 && (
                    <button
                      disabled={isSaving}
                      onClick={goBack}
                      style={{ ...btn('default', isSaving), fontSize: 13 }}
                    >
                      ← 上一步
                    </button>
                  )}
            ...
            return (
              <div style={{ color: clr.danger, fontSize: 13, padding: 8 }}>
                渲染錯誤：{e.message}
              </div>
            );
          }
        ...
        >
          ...
          >
            ...
            {room.lonegirlTransformed &&
              room.lonegirlNewRole &&
              (() => {
                const lonegirlNum = Object.entries(room.roles || {})
                  .find(
                    ([, v]) =>
                      v === room.lonegirlNewRole &&
                      room.lonegirlNewRole !== 'lonegirl'
                  )?.[0]
                  ?.replace('p', '');
                return (
                  <div
                    style={{
                      ...
                      borderRadius: 'var(--border-radius-md)',
                      background:
                        room.lonegirlNewRole === 'wolf'
                          ? clr.dangerBg
                          : clr.infoBg,
                      color:
                        room.lonegirlNewRole === 'wolf' ? clr.danger : clr.info,
                      fontSize: 12,
                    }}
                  >
                    🌟 孤獨少女已轉變為{' '}
                    {ROLE_MAP[room.lonegirlNewRole]?.label ||
                      room.lonegirlNewRole}
            ...
            {night.check?.[0] &&
              (() => {
                const sr = getSeerResult(
                  night.check[0],
                  night.swap?.length === 2 ? night.swap : null,
                  room.roles || {}
                );
                return (
                  <div
...
export default function App() {
  ...
  async function godAction(action, payload = {}) {
    ...
    try {
      ...
      if (!r) {
        setError(
          '無法載入房間資料，請確認 Firebase URL 設定正確，或重新整理後再試'
        );
        return;
      }
      const aliveNums = () =>
      ...
      switch (action) {
        ...
        case 'resolveNight': {
          ...
          if (r.lonegirlIdol && !r.lonegirlTransformed) {
            const idolDied = deaths.find(
              (d) => d.num === Number(r.lonegirlIdol)
            );
            if (idolDied) {
              ...
              const lonegirlAlive =
                ...
                !Object.values(r.players || {}).find(
                  (p) => p.num === Number(lonegirlNum)
                )?.dead;
              if (lonegirlNum && lonegirlAlive && idolRoleId) {
                ...
                if (idolRoleId === 'witch') {
                  ...
                  if (r.witchSaveUsed) {
                    r.log.push(
                      `覺醒孤獨少女（${lonegirlNum} 號）繼承偶像女巫身份，獲得毒藥技能（解藥已耗盡）`
                    );
                  } else {
                    r.witchSaveUsed = false;
                    r.log.push(
                      `覺醒孤獨少女（${lonegirlNum} 號）繼承偶像女巫身份，獲得解藥與毒藥技能`
                    );
                  }
                } else if (idolRoleId === 'seer') {
                  // 繼承預言家：直接繼承查驗能力，無特殊狀態
                  r.log.push(
                    `覺醒孤獨少女（${lonegirlNum} 號）繼承偶像預言家身份，獲得查驗技能`
                  );
                } else if (idolRoleId === 'hunter') {
                  // 繼承獵人：擁有開槍能力（被毒殺才失效，邏輯已在 GodNightPanel 處理）
                  r.log.push(
                    `覺醒孤獨少女（${lonegirlNum} 號）繼承偶像獵人身份，獲得開槍技能`
                  );
                } else if (idolRoleId === 'dreamer') {
                  // 繼承攝夢人：重置 lastDreamTarget（換人攝夢，連續攝夢計數歸零）
                  r.lastDreamTarget = null;
                  r.log.push(
                    `覺醒孤獨少女（${lonegirlNum} 號）繼承偶像攝夢人身份，連續攝夢計數重置`
                  );
                } else {
                  r.log.push(
                    `覺醒孤獨少女（${lonegirlNum} 號）繼承偶像（${
                      r.lonegirlIdol
                    } 號）${ROLE_MAP[idolRoleId]?.label || idolRoleId}身份`
                  );
                }
              }
            }
          ...
          if (wasFirstNight) {
            // First night done → go to sheriff campaign
            r.phase = 'lobby';
            r.log.push('首夜結束，開始警長競選');
          } else {
            // Subsequent nights → go to day
            r.phase = 'day';
            r.exile.votes = {};
            r.exile.result = null;
        ...
        case 'tallyExile': {
          ...
          if (tied.length === 1) {
            ...
            if (
              r.lonegirlIdol &&
              Number(r.lonegirlIdol) === exiled &&
              !r.lonegirlTransformed &&
              lonegirlNum &&
              lonegirlAlive
            ) {
              r.lonegirlTransformed = true;
              ...
              r.log.push(
                `覺醒孤獨少女（${lonegirlNum} 號）的偶像（${exiled} 號）被放逐，孤獨少女轉變為狼人！`
              );
            }
            ...
            if (exiledIsIdiot) {
              ...
              r.exile.result = { exiled, tally, idiotSaved: true };
              ...
              r.phase = 'result';
              r.log.push(
                `${exiled} 號（白癡）被放逐，亮出身份！白癡不出局，但失去投票權`
              );
            } else {
        ...
        case 'startDay': {
          ...
          r.exile.published = false;
          r.exile.targetOptions = aliveNums();
          r.log.push(`第 ${r.dayCount} 天白天，開始放逐投票`);
          break;
        }
        ...
        case 'skipToNight': {
          // Skip day phase entirely → go directly to next night
          r.dayCount++;
          r.phase = 'night';
          r.nightStep = 0;
          r.night = {
          ...
          r.log.push(`略過白天，直接進入第 ${r.dayCount} 夜`);
          break;
        }
      }
  ...
  async function castVote(target) {
    ...
    // 白癡亮牌後不能投票
    if (r.idiotRevealed) {
      const idiotKey = Object.entries(r.roles || {}).find(
        ([, v]) => v === 'idiot'
      )?.[0];
  ...
  if (screen === 'home')
    return (
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '2rem 1rem' }}>
        ...
        <p style={{ color: clr.text2, marginBottom: 16, fontSize: 14 }}>
          12人局 · 線下輔助工具
        </p>
        ...
        {!FIREBASE_CONFIGURED && (
          ...
          >
            ...
            <div style={{ fontSize: 12, color: clr.danger }}>
              請在程式碼頂部將 <code>FIREBASE_URL</code> 換成你的 Firebase
              Realtime Database URL， 否則所有操作將無法儲存。
            </div>
          </div>
        )}
        <div style={card}>
  ...
  // PLAYER VIEW
  // ══════════════════════════════════════════════════════════════════════
  const isAlive = !Object.values(room.players || {}).find(
    (p) => p.num === myNum
  )?.dead;
    ...
    <div style={{ maxWidth: 440, margin: '0 auto', padding: '1.5rem 1rem' }}>
      ...
      {(phase === 'campaignVote' || phase === 'campaignPK') &&
        ...
        (() => {
          ...
          const inelig = isPK
            ...
            : campaign.finalCandidates;
          // 白癡亮牌後失去投票權
          const idiotNum_ = Object.entries(room.roles || {})
            .find(([, v]) => v === 'idiot')?.[0]
            ?.replace('p', '');
          const isIdiotLost =
            room.idiotRevealed && idiotNum_ && Number(idiotNum_) === myNum;
      ...
      {phase === 'day' && (
        <div style={card}>
          ...
          {(() => {
            // 白癡亮牌後失去投票權
            const idiotNum = Object.entries(room.roles || {})
              .find(([, v]) => v === 'idiot')?.[0]
              ?.replace('p', '');
            const isIdiotRevealed =
              room.idiotRevealed && idiotNum && Number(idiotNum) === myNum;
            if (isIdiotRevealed)
              return (
                <div style={{ fontSize: 13, color: clr.warn }}>
                  🃏 白癡身份已揭露，你已失去投票權
    ...
    <div style={{ maxWidth: 560, margin: '0 auto', padding: '1.5rem 1rem' }}>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  cursor: 'pointer',
                  border: 'none',
                  borderLeft: i > 0 ? `0.5px solid ${clr.border2}` : 'none',
              ...
              >
                ...
                {isCurrent && !active && (
                  <span
                    style={{ marginLeft: 5, fontSize: 10, color: clr.warn }}
                  >
                    ●
                  </span>
                )}
        ...
        (room.phase === 'night' ? (
          ...
          <div style={card}>
            ...
            {(room.phase === 'day' || room.phase === 'result') && (
              <button
                disabled={loading}
                style={{ ...btn('warn'), marginTop: 10, width: '100%' }}
                onClick={() => {
                  godAction('skipToNight');
                  setTab('night');
                }}
              >
                ⚡ 略過白天，直接進入夜晚
      ...
      {tab === 'day' && room.phase !== 'lobby' && (
        <div>
          ...
          {/* CAMPAIGN RESULT */}
          {(room.phase === 'campaignVote' || room.phase === 'campaignPK') &&
            campaign.result && (
              <div style={card}>
                <CampaignResult result={campaign.result} />
                <div
